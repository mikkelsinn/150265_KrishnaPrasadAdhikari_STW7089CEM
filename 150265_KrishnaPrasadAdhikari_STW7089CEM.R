...
#Krishna Prasad Adhikari : College ID - 150265
...
#--------------------------------------------------------------------------------
#Import data (X.csv)
Xdata = (read.csv(file = "~/150265_KrishnaPrasadAdhikari_STW7089CEM/X.csv", header = F))
...
#--------------------------------------------------------------------------------
#Import data (Y.csv)
YData = (read.csv(file = "~/150265_KrishnaPrasadAdhikari_STW7089CEM/Y.csv", header = F))
#Convert YData to Matrix
Y = as.matrix(YData)
# Set Column names of Y to Y1
colnames(Y) = c("Y1")
#--------------------------------------------------------------------------------
#Import data (time.csv)
TimeData = (read.csv(file = "~/150265_KrishnaPrasadAdhikari_STW7089CEM/time.csv", header = F))
...
#Density plot of all input signals
plot(density(X), type = "l", xlab = "Input Signals", main = "Density curve of all input Signals")
#Creating histogram of X Signal
hist(X, freq = FALSE, xlab = "Input Signals", main = "Histogram and density curve of all input signals") #plot Histogram of X
#Plot density in histogram
lines(density(X), col = "black", lwd = 1) #Plot density

par(mfrow=c(2,2)) #plots 2x2=4 plots in same display
#Creating histogram of X1 Signal
hist(X[,"X1"], freq = FALSE, xlab = "X1", main = "Histogram and density curve of X1 Signal")
#Plot density in histogram
lines(density(X[,"X1"]), lwd = 1, xlab = "X1")

#Creating histogram of X2 Signal
hist(X[,"X2"], freq = FALSE, xlab = "X2", main = "Histogram and density curve of X2 Signal")
#Plot density in histogram
lines(density(X[,"X2"]), lwd = 1, xlab = "X2")

#Creating histogram of X3 Signal
hist(X[,"X3"], freq = FALSE, xlab = "X3", main = "Histogram and density curve of X3 Signal")
#Plot density in histogram
lines(density(X[,"X3"]), lwd = 1, xlab = "X3")

#Creating histogram of X4 Signal
hist(X[,"X4"], freq = FALSE, xlab = "X4", main = "Histogram and density curve of X4 Signal")
#Plot density in histogram
...
par(mfrow=c(2,2)) #plots 2x2=4 plots in same display
#Creating Plot
plot(X[,"X1"], Y, pch = 1, col="blue", main = "Correlation between X1 and Y", xlab = "X1")
#regression line
abline(lm(Y ~ X[,"X1"]), col = "red", lwd = 1)
...
#Creating Plot
plot(X[,"X3"], Y, pch = 1, col="blue", main = "Correlation between X3 and Y", xlab = "X3")
#regression line
abline(lm(Y ~ X[,"X3"]), col = "red", lwd = 1)

#Creating Plot
plot(X[,"X4"], Y, pch = 1, col="blue", main = "Correlation between X4 and Y", xlab = "X4")
#regression line
abline(lm(Y ~ X[,"X4"]), col = "red", lwd = 1)
...
XData_model1 = cbind(onesMatrix, X[,"X4"], X[,"X1"]^2, X[,"X1"]^3, X[,"X2"]^4, X[,"X1"]^4)
XData_model1
# Creating Data for model 2 from given equation
XData_model2 = cbind(onesMatrix, X[,"X4"], X[,"X1"]^3, X[,"X3"]^4)
XData_model2
# Creating Data for model 3 from given equation
XData_model3 = cbind(onesMatrix, (X[,"X3"])^3, (X[,"X3"])^4)
XData_model3
# Creating Data for model 4 from given equation
XData_model4 = cbind(onesMatrix, X[,"X2"], (X[,"X1"])^3, (X[,"X3"])^4)
XData_model4
# Creating Data for model 5 from given equation
XData_model5 = cbind(onesMatrix, (X[,"X4"]), (X[,"X1"])^2, (X[,"X1"])^3, (X[,"X3"])^4)
XData_model5
#--------------------------------------------------------------------------------
#Task 2.1: 
#--------------------------------------------------------------------------------
...
# Selecting Model 2, estimating model parameters using training data
TrainingOneXMatrix = matrix(1, length(X_Training_Set$X1), 1) # ones matrix for training set
TrainingXModel = cbind(TrainingOneXMatrix, X_Training_Set[, "X4"], (X_Training_Set[, "X1"])^3, (X_Training_Set[, "X3"])^4) #Training Model
...
# Model 2 thetahat values from Task 2.1
ThetaBias = 0.483065688 # chosen parameter
ThetaA = 0.143578928 # chosen parameter
ThetaB = 0.010038614 # set constant
ThetaC = 0.001912836 # set constant
Epsilon = RSS_model2 * 2 ## fixing value of epsilon, RSS_model2 from task 2.2
